...
subtitle: "Influence between sex and weight"
author: "Shuhan Song"
date: "1/25/2020"
...
#### Track female and male


```{r}
# get average mean weight for both male and female through time
snowshoe_weight_time <- snowshoe_clean %>% 
  group_by(date, sex) %>% 
  summarize(weight = mean(weight, na.rm = TRUE))
```

```{r, eval=FALSE}
skim(snowshoe_weight_time) # check the number of missing data
```

```{r}
# format date column
snowshoe_weight_time_rmna <- snowshoe_weight_time %>% 
  drop_na(weight) %>% 
  ungroup()  %>% 
  mutate(date = mdy(date)) %>%  # convert mmddYY to YYYYmmdd
  mutate(ym = month)

ggplot(data = snowshoe_weight_time_rmna,
       aes(x = date,
           y = weight)) +
